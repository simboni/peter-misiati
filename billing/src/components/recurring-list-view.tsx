  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2.5 md:hidden">
          ...
          <div key={r.id} className="card p-4">
            <div className="flex items-start gap-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted">{r.frequencyLabel} · next {r.status === "ended" ? "—" : r.nextRun}</div>
              </div>
              <div className="flex flex-col items-end gap-1">
                <div className="font-medium text-ink tabular-nums">{formatMoney(r.total, r.currency)}</div>
        ...
        {filtered.length === 0 && (
          <div className="card p-6 text-center text-sm text-muted">No schedules match your filters.</div>
        )}
      </div>
