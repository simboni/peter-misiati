  ...
  */
  ...
  const parts = mix.ingredients.map<ResolvedLine>((ing) => {
    ...
    return {
      ...
      lineTotalCents: 0,
      listPriceCents: 0,
      costCents: Math.round((item.cost_cents * ing.qtyMilli) / MILLI),
      formulaVersionId: version.id,
      bundleId: null,
  ...
  return input.lines.flatMap((line) => {
    ...
    */
    if (line.bundleId != null) {
      ...
      return {
        ...
        listPriceCents: bundle.priceCents,
        costCents: Math.round((item.cost_cents * qtyMilli) / MILLI),
    ...
    */
    ...
    return {
      ...
      lineTotalCents: line.unitPriceCents * line.units,
      listPriceCents,
      costCents: Math.round((item.cost_cents * item.size_milli * line.units) / MILLI),
      formulaVersionId,
      bundleId: null,
      nameSnapshot: item.name,
    };
