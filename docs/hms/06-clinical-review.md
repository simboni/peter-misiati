...
| 21.9 | A code lives ten minutes and dies after three wrong tries, and is stored under **scrypt with a per-code salt** rather than a plain digest. A six-digit code has a million possibilities and a million SHA-256 operations take under a second, so a "hashed" code in a stolen table is not hashed in any useful sense — measured at 0.9 seconds for the whole keyspace against 9.6 hours under scrypt. Comparison is constant-time | `portal.ts` `sealCode`, `codeMatches` | Design rule | ✅ |
